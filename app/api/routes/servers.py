...
async def update_server(server_id: str, db: AsyncSession = Depends(get_db)) -> ServerResponse:
    ...
    if not Path(server.installation_path).exists():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Server installation path does not exist: {server.installation_path}. The server may need to be reinstalled.",
        )

    if not server.installation_path:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Server installation path not found. The server may not have been installed yet or installation failed.",
        )
    ...
    # Disconnect server before update
    southbound_manager.set_db_session(db)
    await southbound_manager.disconnect_server(server_id)
